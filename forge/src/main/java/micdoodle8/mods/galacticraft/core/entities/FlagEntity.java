...
{
    private static final EntityDataAccessor<String> OWNER = SynchedEntityData.defineId(FlagEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Float> DAMAGE = SynchedEntityData.defineId(FlagEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> FACING_ANGLE = SynchedEntityData.defineId(FlagEntity.class, EntityDataSerializers.INT);
    ...
    public FlagEntity(EntityType<FlagEntity> type, Level world)
    {
        super(type, world);
        this.noCulling = true;
    }

    public FlagEntity(Level world, double x, double y, double z, int dir)
    {
    ...
    public boolean hurt(DamageSource par1DamageSource, float par2)
    {
        ...
        if (!this.level.isClientSide && this.isAlive() && !this.indestructable)
        {
            ...
            else
            {
                ...
                this.level.playSound(null, this.getX(), this.getY(), this.getZ(), SoundType.METAL.getBreakSound(), SoundCategory.BLOCKS, SoundType.METAL.getVolume(), SoundType.METAL.getPitch() + 1.0F);
