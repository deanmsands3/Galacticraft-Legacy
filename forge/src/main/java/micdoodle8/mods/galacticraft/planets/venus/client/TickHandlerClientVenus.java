        ...
        if (player == event.player)
        {
            if (!ConfigManagerPlanets.disableAmbientLightning.get())
            {
                ...
                if (player.getRandom().nextInt(300 + (int) (800F * minecraft.level.rainLevel)) == 0 && minecraft.level.getDimension() instanceof DimensionVenus)
                {
                    ...
                    minecraft.level.playSound(player, posX, posY, posZ, SoundEvents.LIGHTNING_BOLT_THUNDER, SoundCategory.WEATHER, 500.0F + player.getRandom().nextFloat() * 500F, 1.0F + player.getRandom().nextFloat() * 0.2F);
