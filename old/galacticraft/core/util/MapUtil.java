...
public class MapUtil
{
    ...
    public static void resetClientBody()
    {
        ...
        GalacticraftCore.packetPipeline.sendToServer(new PacketSimple(PacketSimple.EnumSimplePacket.S_REQUEST_OVERWORLD_IMAGE, GCCoreUtil.getDimensionType(Minecraft.getInstance().level), new Object[]{}));
            ...
            for (int z0 = -12; z0 <= 12; z0++)
            {
                ...
                if (chunk != null)
                {
                    for (int z = 0; z < 16; z++)
                    {
                        for (int x = 0; x < 16; x++)
                        {
                            int l4 = chunk.getHeight(Heightmap.Type.WORLD_SURFACE_WG, x, z) + 1;
    ...
    private static boolean makeRGBimage(NativeImage image, File baseFolder, int cx, int cz, int offsetX, int offsetZ, int xCoord, int zCoord, DimensionType dim, boolean prevResult)
    {
        ...
        if (!filename.exists())
        {
            ...
            else
            {
                ...
                GalacticraftCore.packetPipeline.sendToServer(new PacketSimple(PacketSimple.EnumSimplePacket.S_REQUEST_MAP_IMAGE, GCCoreUtil.getDimensionType(Minecraft.getInstance().level), new Object[]{dim, cx, cz}));
            }
            return true;
        }
