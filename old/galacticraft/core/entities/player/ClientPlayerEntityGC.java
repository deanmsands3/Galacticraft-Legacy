    ...
    public void aiStep()
    {
        ...
        try
        {
            if (this.level.getDimension() instanceof IZeroGDimension)
            {
                ...
                this.input.func_225607_a_(this.func_228354_I_());
                ...
                if (this.isSprinting()) {
                    boolean flag5 = !this.input.func_223135_b() || !flag4;
                ...
                if (this.input.jumping && !flag7 && !flag && !this.abilities.flying && !this.isPassenger() && !this.onLadder()) {
                    ...
                    if (itemstack.getItem() == Items.ELYTRA && ElytraItem.isFlyEnabled(itemstack) && this.tryToStartFallFlying()) {
                        this.connection.send(new ServerboundPlayerCommandPacket(this, CEntityActionPacket.Action.START_FALL_FLYING));
                    }
                }
...
//                }
    ...
    private boolean movingForward() {
        double d0 = 0.8D;
        return this.isUnderWater() ? this.input.func_223135_b() : (double)this.input.forwardImpulse >= 0.8D;
    }
