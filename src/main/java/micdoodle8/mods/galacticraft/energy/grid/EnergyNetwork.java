package micdoodle8.mods.galacticraft.energy.grid;
...
public class EnergyNetwork implements IElectricityNetwork
{
    ...
    private float doProduce()
    {
        ...
        if (!this.availableAcceptors.isEmpty())
        {
            ...
            {
                GCLog.warn("DEBUG Energy network loop issue, please report this");
                if (debugTE instanceof TileEntity)
                {
                    GCLog.warn("Problem was likely caused by tile in dim " + GCCoreUtil.getDimensionID(((TileEntity)debugTE).getWorld()) + " at " + ((TileEntity)debugTE).getPos() + " Type:" + debugTE.getClass().getSimpleName());
                }
            }
        }
