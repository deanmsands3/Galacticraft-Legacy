package micdoodle8.mods.galacticraft.world;
...
public class ChunkLoadingCallback implements LoadingCallback
{
    ...
    public static void loadConfig(File file)
    {
        ...
        {
            GCLog.warn("Problem loading chunkloading config (\"core.conf\")");
        }
        finally
        {
        ...
        if (saveDir != null)
        {
            ...
            if (!saveFile.exists())
            {
                try
                {
                    ...
                    {
                        GCLog.warn("Could not create chunk loader data file: " + saveFile.getAbsolutePath());
                    }
                }
                catch (IOException e)
                {
    ...
    private static File getSaveDir()
    {
        if (DimensionManager.getWorld(0) != null)
        {
            ...
            if (!saveDir.exists())
            {
                if (!saveDir.mkdirs())
                {
                    GCLog.warn("Could not create chunk loader save data folder: " + saveDir.getAbsolutePath());
                }
            }
