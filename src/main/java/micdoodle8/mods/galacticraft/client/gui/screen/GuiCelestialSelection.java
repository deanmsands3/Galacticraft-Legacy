            ...
            if (this.selectedBody.getReachable() && this.possibleBodies != null && this.possibleBodies.contains(this.selectedBody))
            {
                try
                {
                    ...
                    if (this.selectedBody instanceof Satellite)
                    {
                        ...
                        {
                            GCLog.warn("Please report as a BUG: spaceStationIDs was null.");
                            return;
                        }
                        ...
                        if (mapping == null)
                        {
                            GCLog.warn("Problem matching player name in space station check: " + this.selectedStationOwner);
                            return;
                        }
                        dimensionID = mapping;
                        ...
                        {
                            GCLog.warn("Failed to find a spacestation with dimension " + dimensionID);
                            return;
                        }
                    }
                    else
    ...
    public void drawScreen(int mousePosX, int mousePosY, float partialTicks)
    {
        ...
        } catch (Exception e)
        {
            if (!this.errorLogged)
            {
                ...
                GCLog.warn("Problem identifying planet or dimension in an add on for Galacticraft!");
                GCLog.warn("(The problem is likely caused by a dimension ID conflict.  Check configs for dimension clashes.  You can also try disabling Mars space station in configs.)");
                e.printStackTrace();
            }
        }
