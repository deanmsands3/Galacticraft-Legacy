...
public class MapGen extends BiomeProvider implements Runnable
{
    ...
    public MapGen(World worldIn, int sx, int sz, int cx, int cz, int scale, File file)
    {
        ...
        {
            GCLog.warn("Galacticraft background map image generator not able to run (probably a mod conflict?)");
            GCLog.warn("Please report this at https://github.com/micdoodle8/Galacticraft/issues/2481");
            e.printStackTrace();
            this.mapNeedsCalculating = false;
    ...
    public boolean BiomeMapOneTick()
    {
        ...
        {
            GCLog.warn("Galacticraft background map image generator hit an error (probably a mod conflict?)");
            GCLog.warn("--> Please report this at https://github.com/micdoodle8/Galacticraft/issues/2544 <--");
            e.printStackTrace();
            MapGen.disabled = true;
            this.aborted.set(true);
            return true;
