...
//            
        ...
        {
            GCLog.warn("Problem loading core config (\"core.conf\")");
            e.printStackTrace();
        }
    }
    ...
    public static BlockTuple stringToBlock(String s, String caller, boolean logging)
    {
        ...
        if (block == null)
        {
            ...
            if (block == null)
            {
                if (logging)
                {
                    GCLog.warn("[config] " + caller + ": unrecognised block name '" + s + "'.");
                }
                return null;
            }
        }
    ...
    private static int parseKeyValue(String key)
    {
        ...
        GCLog.warn("Failed to parse keyboard key: " + key + "... Use values A-Z or 0-9");
