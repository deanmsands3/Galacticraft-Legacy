    ...
    public void run()
    {
        ...
        while (this.count < 3 && remotePatchVer == -1)
        {
                ...
                while ((str = in.readLine()) != null)
                {
                    ...
                    if (str.contains("Version"))
                    {
                        ...
                        if (remoteMajVer == Constants.LOCALMAJVERSION && (remoteMinVer > Constants.LOCALMINVERSION || (remoteMinVer == Constants.LOCALMINVERSION && remotePatchVer > Constants.LOCALPATCHVERSION)))
                        {
                            ...
                            {
                                GCLog.warn("New Galacticraft version available! v" + String.valueOf(remoteMajVer) + "." + String.valueOf(remoteMinVer) + "." + String.valueOf(remotePatchVer) + ".xxx" + " http://micdoodle8.com/");
                            }
                        }
                        break;
                    }
            ...
            if (remotePatchVer == -1)
            {
                try
                {
                    GCLog.warn(GCCoreUtil.translate("newversion.failed.name"));
                    Thread.sleep(15000);
                }
