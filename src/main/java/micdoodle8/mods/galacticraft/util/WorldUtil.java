...
import micdoodle8.mods.galacticraft.entities.EntityCelestialFake;
...
import micdoodle8.mods.galacticraft.tile.TileEntityTelemetry;
            ...
            for (File var5 : var2)
            {
                if (var5.getName().startsWith("spacestation_") && var5.getName().endsWith(".dat"))
                {
                    try
                    {
                        ...
                        {
                            GCLog.warn("Dimension already registered to another mod: unable to register space station dimension " + registeredID);
                        }
                    }
                    catch (Exception e)
                    {
...
//        }
    ...
    public static boolean registerPlanet(int id, boolean initialiseDimensionAtServerInit, int defaultID)
    {
        ...
        if (initialiseDimensionAtServerInit)
        {
            ...
            else
            {
                ...
                {
                    GCLog.warn("Dimension already registered: unable to register planet dimension " + id);
    ...
    public static void registerPlanetClient(Integer dimID, int providerIndex)
    {
        ...
        if (typeID == 0)
        {
            GCLog.warn("Server dimension " + dimID + " has no match on client due to earlier registration problem.");
        }
        else if (dimID == 0)
        {
            GCLog.warn("Client dimension " + providerIndex + " has no match on server - probably a server dimension ID conflict problem.");
        }
        else

        {
            ...
            {
                GCLog.warn("Dimension already registered to another mod: unable to register planet dimension " + dimID);
            }
        }
    }
    ...
    public static SpaceStationWorldData createSpaceStation(World world, int dimID, int homePlanetID, int dynamicProviderID, int staticProviderID, EntityPlayerMP player)
    {
        ...
        {
            GCLog.warn("Dimension already registered to another mod: unable to register space station dimension " + dimID);
        }
    ...
    public static Entity transferEntityToDimension(Entity entity, int dimensionID, WorldServer world, boolean transferInv, EntityAutoRocket ridingRocket)
    {
        if (!world.isRemote)
        {
            ...
            if (server != null)
            {
                final WorldServer var6 = server.getWorld(dimensionID);
    ...
    public static WorldServer getStartWorld(WorldServer unchanged)
    {
        if (ConfigManagerCore.challengeSpawnHandling)
        {
            ...
            if(wp != null) {
            	WorldProviderSpace provider = (WorldProviderSpace) wp;
            	WorldServer newWorld = (WorldServer) provider.getWorld();
...
//                    }
    ...
    private static void registerSSdim(Integer dimID, Integer providerKey)
    {
        if (!WorldUtil.registeredSpaceStations.containsKey(dimID))
        {
            ...
            {
                GCLog.warn("Dimension already registered on client: unable to register space station dimension " + dimID);
            }
        }
    }
    ...
    public static DimensionType getDimensionTypeById(int id)
    {
        ...
        GCLog.warn("There was a problem getting WorldProvider type " + id);
        GCLog.warn("(possibly this is a conflict, check Galacticraft config.)");
        return null;
    }
