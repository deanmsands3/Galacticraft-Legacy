package micdoodle8.mods.galacticraft.dimension;
...
import micdoodle8.mods.galacticraft.blocks.BlockSpinThruster;
import micdoodle8.mods.galacticraft.entities.ITumblable;
import micdoodle8.mods.galacticraft.entities.player.FreefallHandler;
...
public class SpinManager
{
    ...
    public boolean refresh(BlockPos baseBlock, boolean placingThruster)
    {
        ...
        IBlockState bsStart = this.worldProvider.getWorld().getBlockState(this.oneSSBlock);
            ...
            for (BlockVec3 vec : currentLayer)
            {
                ...
                for (int side = 0; side < 6; side++)
                {
                    ...
                    if (sideVec != null && !this.checked.contains(sideVec))
                    {
                        ...
                        IBlockState state = sideVec.getBlockState(this.worldProvider.getWorld());
                        if (state == null)
                        {
                        	continue;
                        }
                        ...
                        if (b != null && !b.isAir(state, this.worldProvider.getWorld(), sideVec.toBlockPos()) && !(b instanceof BlockDynamicLiquid))
                        {
                            ...
                            if (!(b instanceof BlockLiquid))
                            {
                            	...
                            	m = state.getBlockHardness(this.worldProvider.getWorld(), sideVec.toBlockPos());
                                if (m < 0.1F)
                                {
                                    m = 0.1F;
                                }
        ...
        if (placingThruster && !this.checked.contains(new BlockVec3(baseBlock)))
        {
            ...
            {
                this.oneSSBlock = baseBlock;
                IBlockState state = this.worldProvider.getWorld().getBlockState(this.oneSSBlock);
        ...
        {
            IBlockState state = this.worldProvider.getWorld().getBlockState(thruster);
