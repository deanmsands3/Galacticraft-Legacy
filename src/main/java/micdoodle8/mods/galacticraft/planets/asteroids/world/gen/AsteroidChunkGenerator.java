...
//    final byte ASTEROID_STONE_META_2 = 2;
    private final BlockState ASTEROID_STONE_0 = AsteroidBlocks.rock0.defaultBlockState();
    private final BlockState ASTEROID_STONE_1 = AsteroidBlocks.rock1.defaultBlockState();
    private final BlockState ASTEROID_STONE_2 = AsteroidBlocks.rock2.defaultBlockState();
...
//        this.rand = new Random(par2);
        ...
        {
            this.coreHandler.addBlock(new SpecialAsteroidBlock(AsteroidBlocks.oreAluminum.defaultBlockState(), 5, .2));
        }
        if (!ConfigManagerPlanets.disableIlmeniteGenAsteroids.get())
        {
            this.coreHandler.addBlock(new SpecialAsteroidBlock(AsteroidBlocks.oreIlmenite.defaultBlockState(), 4, .15));
        }
        if (!ConfigManagerPlanets.disableIronGenAsteroids.get())
        {
            this.coreHandler.addBlock(new SpecialAsteroidBlock(AsteroidBlocks.oreIron.defaultBlockState(), 3, .2));
        }
        if (ConfigManagerCore.enableSiliconOreGen.get())
        {
            this.coreHandler.addBlock(new SpecialAsteroidBlock(GCBlocks.oreSilicon.defaultBlockState(), 2, .15)); //TODO: Asteroids version of silicon ore
        }
        //Solid Meteoric Iron - has no config to disable
        this.coreHandler.addBlock(new SpecialAsteroidBlock(GCBlocks.decoBlockMeteorIron.defaultBlockState(), 2, .13));
        ...
        this.shellHandler.addBlock(new SpecialAsteroidBlock(AsteroidBlocks.blockDenseIce.defaultBlockState(), 1, .15));
    }
    ...
    private void generateAsteroid(Random rand, int asteroidX, int asteroidY, int asteroidZ, int chunkX, int chunkZ, int size, ProtoChunk primer, boolean flagDataOnly)
    {
        ...
        {
            isHollow = true;
            shell = new SpecialAsteroidBlock(AsteroidBlocks.blockDenseIce.defaultBlockState(), 1, .15);
        }
...
//                    meta = this.ASTEROID_STONE_META_1;
                    ...
                    {
                        state = AsteroidBlocks.oreIlmenite.defaultBlockState();
                    ...
                    {
                        state = AsteroidBlocks.oreIron.defaultBlockState();
                    ...
                    {
                        state = AsteroidBlocks.oreAluminum.defaultBlockState();
...
//                            chunk.setBlockState(new BlockPos(x - 1, y, z), GCBlocks.brightAir.getStateFromMeta(13 - count)); TODO ? Has bright air ever had metadata?
                            chunk.setBlockState(new BlockPos(x - 1, y, z), GCBlocks.brightAir.defaultBlockState(), false);
