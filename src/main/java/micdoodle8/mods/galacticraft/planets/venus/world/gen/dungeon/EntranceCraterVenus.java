...
public class EntranceCraterVenus extends SizedPieceVenus
{
    ...
    public EntranceCraterVenus(StructurePieceType type, CompoundTag nbt)
    {
        super(type, nbt);
    }

    public EntranceCraterVenus(StructurePieceType type, DungeonConfigurationVenus configuration, Random rand, int blockPosX, int blockPosZ)
    {
        super(type, configuration, rand.nextInt(4) + 6, 12, rand.nextInt(4) + 6, Direction.Plane.HORIZONTAL.getRandomDirection(rand));
...
//                        this.setBlockState(worldIn, Blocks.AIR.getDefaultState(), i + range, j, k + range, boundingBox);
    ...
    {
        return new RoomEntranceVenus(this.configuration, rand, this.boundingBox.x0 + this.boundingBox.getXSpan() / 2, this.boundingBox.z0 + this.boundingBox.getZSpan() / 2);
    }
}
