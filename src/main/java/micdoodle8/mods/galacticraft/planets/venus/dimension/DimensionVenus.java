...
//    }
    ...
    {
        if ((int) yy >= blockpos.getY() + 1 && world.getHeightmapPos(Heightmap.Types.MOTION_BLOCKING, blockpos).getY() > blockpos.getY())
        {
            mc.level.playLocalSound(xx, yy, zz, SoundEvents.LAVA_EXTINGUISH, SoundSource.WEATHER, 0.025F, 0.6F + random.nextFloat() * 0.2F, false);
        }
        else
        {
            mc.level.playLocalSound(xx, yy, zz, SoundEvents.LAVA_EXTINGUISH, SoundSource.WEATHER, 0.04F, 0.8F + random.nextFloat() * 0.06F + random.nextFloat() * 0.06F, false);
        }
    }
