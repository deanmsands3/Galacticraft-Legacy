...
public class EntitySpiderQueen extends EntityBossBase implements IEntityBreathable, IBoss, RangedAttackMob
{
    private static final EntityDataAccessor<Byte> BURROWED_COUNT = SynchedEntityData.defineId(EntitySpiderQueen.class, EntityDataSerializers.BYTE);
    ...
    public EntitySpiderQueen(EntityType<? extends EntitySpiderQueen> type, Level worldIn)
    {
        super(type, worldIn);
//        this.setSize(1.4F, 0.9F);
    ...
    public static EntitySpiderQueen create(Level world)
    {
    ...
    public void tick()
    {
        ...
        if (this.spawnedPreload != null)
        {
            ...
            {
                Entity entity = null;
                Optional<Entity> first = ((ServerLevel) this.level).getEntities().filter((e) -> e.getUuid().equals(id)).findFirst();
                if (first.isPresent())
                {
                    entity = first.get();
                }
                    ...
                    if (Math.abs(dY) - this.getBbHeight() < 1.1 && Math.abs(this.getY() - this.yOld) < 0.05)
                    {
                        if (this.getBurrowedCount() >= 0)
                        {
                            if (this.tickCount % 20 == 0)
                            {
                                ...
                                {
                                    EntityJuicer juicer = new EntityJuicer(VenusEntities.JUICER, this.level);
...
//    }
    ...
    {
        EntityWebShot entityarrow = EntityWebShot.createEntityWebShot(this.level, this, target, 0.8F, (float) (14 - this.level.getDifficulty().getId() * 4));
    ...
    public void dropKey()
    {
        this.spawnAtLocation(new ItemStack(VenusItems.keyT3, 1), 0.5F);
    }
