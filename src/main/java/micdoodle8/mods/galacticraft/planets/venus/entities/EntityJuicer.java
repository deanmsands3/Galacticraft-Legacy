...
public class EntityJuicer extends Monster implements IEntityBreathable
{
    private static final EntityDataAccessor<Boolean> IS_FALLING = SynchedEntityData.defineId(EntityJuicer.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> IS_HANGING = SynchedEntityData.defineId(EntityJuicer.class, EntityDataSerializers.BOOLEAN);
    private BlockPos jumpTarget;
    private int timeSinceLastJump = 0;

    public EntityJuicer(EntityType<? extends EntityJuicer> type, Level worldIn)
    {
        super(type, worldIn);
        ...
        if (!this.level.isClientSide)
        {
            if (this.jumpTarget == null)
            {
                if (this.timeSinceLastJump <= 0)
                {
                    ...
                    {
                        HitResult hit = this.level.clip(new ClipContext(new Vec3(this.getX(), this.getY(), this.getZ()), new Vec3(this.getX(), this.getY() + (this.isHanging() ? -10 : 10), this.getZ()), ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, this));
