    ...
    public void onWorldTick(TickEvent.WorldTickEvent event)
    {
        if (event.side == LogicalSide.SERVER && event.phase == TickEvent.Phase.START)
        {
            ((ServerLevel) event.world).getEntities()
                    .filter((e) -> e.age % 20 == 1 && e instanceof LivingEntity && event.world.containsBlockWithMaterial(e.getBoundingBox().expand(-0.1D, -0.4D, -0.1D), PlanetFluids.ACID_MATERIAL))
                    .forEach((e) -> e.damage(DamageSourceGC.acid, 3.0F));
        }
    }
...
//        }
    ...
    public void onEntitySpawned(EntityJoinWorldEvent event)
    {
        ...
        {
            for (BlockEntity tile : event.getEntity().getEntityWorld().blockEntities)
            {
