...
public class EntityProjectileTNT extends AbstractHurtingProjectile
{
    public EntityProjectileTNT(EntityType<? extends EntityProjectileTNT> type, Level worldIn)
    {
        super(type, worldIn);
//        this.setSize(1.0F, 1.0F);
    }

    public static EntityProjectileTNT createEntityProjectileTNT(Level world, LivingEntity entityShooting, double motX, double motY, double motZ)
    {
...
//    }
    ...
    protected void onHit(HitResult movingObjectPosition)
    {
        if (!this.level.isClientSide)
        {
            ...
            this.level.explode(null, this.getX(), this.getY(), this.getZ(), 1.0F, false, this.level.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING) ? Explosion.BlockInteraction.BREAK : Explosion.BlockInteraction.NONE);
            this.remove();
        }
    }
