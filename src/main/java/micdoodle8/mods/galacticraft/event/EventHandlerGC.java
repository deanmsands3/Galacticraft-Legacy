package micdoodle8.mods.galacticraft.event;
...
import micdoodle8.mods.galacticraft.TransformerHooks;
import micdoodle8.mods.galacticraft.client.SkyProviderOverworld;
import micdoodle8.mods.galacticraft.client.gui.container.GuiPositionedContainer;
...
import micdoodle8.mods.galacticraft.entities.EntityLanderBase;
import micdoodle8.mods.galacticraft.entities.EntityMeteor;
...
//                event.entity.move(event.entity.motionX, event.entity.motionY, event.entity.motionZ);
    ...
    //			String s1 = String.format("%s:%s:%s/%s", new Object[] { "mcsounddomain", resourcelocation.getNamespace(), "sound", resourcelocation.getPath() });
            ...
            if (event.getResultSound().getAttenuationType() != ISound.AttenuationType.NONE)
            {
                ...
                if (gearData == null || gearData.getFrequencyModule() == -1)
                {
                    ...
                    if ((!playerInAtmosphere || !soundInAtmosphere))
                    {
                        ...
                        {
                            GCLog.warn("Sound event null! " + event.getName() + " " + event.getResultSound().getSoundLocation());
                        }
                    }
                }
            }
