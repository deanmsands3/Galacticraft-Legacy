function initializeCoreMod() {
    ...
    var TRANSFORMER_HOOKS_CLASS = "micdoodle8/mods/galacticraft/core/TransformerHooks";

    return {
         // Redirect living entity gravity to hooks class
		 "LivingEntity.travel": {
		 	"target": {
                "type": "METHOD",
                "class": "net.minecraft.entity.LivingEntity",
                "methodName": "func_213352_e",
                "methodDesc": "(Lnet/minecraft/util/math/Vec3d;)V"
		 	},
		 	"transformer": function(methodNode) {
                var instructions = methodNode.instructions;
                var getValue_owner = "net/minecraft/entity/ai/attributes/IAttributeInstance";
                var getValue_name = ASMAPI.mapMethod("func_111126_e"); // IAttributeInstance#getValue
                var getValue_desc = "()D";

                var first_INVOKEINTERFACE_getValue;
                var arrayLength = instructions.size();
                for (var i = 0; i < arrayLength; ++i) {
                    var instruction = instructions.get(i);
                    if (instruction.getOpcode() == INVOKEINTERFACE) {
                        if (instruction.owner == getValue_owner) {
                            if (instruction.name == getValue_name) {
                                if (instruction.desc == getValue_desc) {
                                    first_INVOKEINTERFACE_getValue = instruction;
                                    print("[GC] Found injection point \"first_INVOKEINTERFACE_getValue\" " + instruction);
                                    break;
                                }
                            }
                        }
                ...
                if (!first_INVOKEINTERFACE_getValue)
                    throw "[GC] Error: Couldn't find injection point \"IAttributeInstance.getValue()\"!";

                var toInject = new InsnList();

                toInject.add(new VarInsnNode(ALOAD, 0)); // this
                toInject.add(new MethodInsnNode(INVOKESTATIC, TRANSFORMER_HOOKS_CLASS, "getGravityForEntity", "(Lnet/minecraft/entity/Entity;)D"));

                instructions.insert(first_INVOKEINTERFACE_getValue, toInject);
