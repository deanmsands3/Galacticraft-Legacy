...
public class BiomeAdaptive extends GalacticraftBiome
{
    public static BiomeAdaptive biomeDefault;
    public static List<BiomeAdaptive> biomeList = new LinkedList<>(); 
    private GalacticraftBiome biomeTrue;
    private final int index;
    private boolean loggedConflict;
    
    BiomeAdaptive(int i, GalacticraftBiome biomeInitial)
    {
        super(BiomeData.builder().name("Outer Space" + (i == 0 ? "" : " "+ i)).build());
        this.index = i;
    ...
    {
        if (this.biomeTrue instanceof GalacticraftBiome)
        {   
            ((GalacticraftBiome) this.biomeTrue).registerTypes(this);
        }
        else
        {
    ...
    public static GalacticraftBiome register(int index, GalacticraftBiome biome)
    {
    ...
    public static GalacticraftBiome getDefaultBiomeFor(CelestialBody body)
    {
    ...
    private void reportBiomeIDconflict()
    {
        ...
        GCLog.warn("POTENTIAL BIOME ID CONFLICT for id " + Biome.getIdForBiome(this) + " conflicting with Galacticraft");
        GCLog.warn("PLEASE CHECK CONFIGS FOR BOTH MODS: see Galacticraft core.conf setting BiomeIDBase");
        Thread.dumpStack();
    }
}
