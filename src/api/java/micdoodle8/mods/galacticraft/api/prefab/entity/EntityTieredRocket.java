        ...
        if (this.destinationFrequency != -1)
        {
            ...
            if (this.targetVec != null)
            {
                if (this.targetDimension != this.world.provider.getDimension())
                {
                    ...
                    if(targetDim != null && targetDim instanceof WorldProviderSpace) {
                    	
                    	WorldProviderSpace target = (WorldProviderSpace) targetDim;
                    	
                    	if(target.getWorld() instanceof WorldServer) {
                    		
                        	boolean dimensionAllowed = this.targetDimension == ConfigManagerCore.idDimensionOverworld;

                        	if (target instanceof IGalacticraftWorldProvider)
                        	{
                        		if (((IGalacticraftWorldProvider) target).canSpaceshipTierPass(this.getRocketTier()))
                        	...
                        	if (dimensionAllowed)
                        	{
                        		if (!this.getPassengers().isEmpty())
                        		{
                        		    for (Entity passenger : this.getPassengers())
                        		    {
                        		        ...
                        		        {
                        		            WorldUtil.transferEntityToDimension(passenger, this.targetDimension, (WorldServer) target.getWorld(), false, this);
                        		        }
                        		    }
                        		}
                        		else
                        		{
                        		    Entity e = WorldUtil.transferEntityToDimension(this, this.targetDimension, (WorldServer)target.getWorld(), false, null);
